...
|------|------|----|
| 온보딩 완료 | 닉네임 확정 시 | +500 |
| 퀴즈 정답 (상) | 1회차 퀴즈팩, 정답 시 | +30 |
| 퀴즈 정답 (중상) | | +25 |
| 퀴즈 정답 (중) | | +20 |
| 퀴즈 정답 (중하) | | +15 |
| 퀴즈 정답 (하) | | +10 |
| 힌트 사용 (상) | 힌트 버튼 클릭 시 | -30 |
| 힌트 사용 (중상) | | -25 |
| 힌트 사용 (중) | | -20 |
| 힌트 사용 (중하) | | -15 |
| 힌트 사용 (하) | | -10 |
...
### 제외 조건
- 완료한 퀴즈팩의 **다시 풀기**(2회차 이상)는 XP 지급/차감 제외
...
| `quizpack_complete` | 퀴즈팩 1회차 완료 XP 확정 | +180 (해당 팩 총합) | `true` (리그 대상) |
...
```
session_number >= 2인 경우:
  → XP 관련 로직 전체 스킵
  → pending_xp 계산 안 함
  → users_xp_history 기록 안 함
```
...
7. **다시 풀기 제외**: 완료된 퀴즈팩 다시 풀기 → XP 변동 없음 확인
